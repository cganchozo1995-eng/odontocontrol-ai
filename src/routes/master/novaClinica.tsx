...
const schema = z.object({
  ...
  slug: z.string().regex(/^[a-z0-9-]*$/, "Use minúsculas, números y guion").max(60).optional().or(z.literal("")),
  ...
  email_admin: z.string().email("Correo inválido"),
...
function Page() {
  ...
  const onSubmit = async (v: FormV) => {
    ...
    try {
      ...
      toast.error(e?.message ?? "Error al crear clínica");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <>
      <PageHeader title="Nueva clínica" description="Registro completo + creación de admin" />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-5xl">
        ...
        <Section icon={UserCog} title="Admin de acceso">
          <Grid>
          ...
          <p className="text-xs text-muted-foreground mt-3">Se generará automáticamente una contraseña aleatoria segura. Aparece UNA vez al final del registro.</p>
        </Section>
        ...
        <div className="flex justify-end gap-2 sticky bottom-0 bg-background/80 backdrop-blur p-3 -mx-3 border-t">
          ...
          <Button type="submit" disabled={busy}>{busy && <Loader2 className="size-4 animate-spin mr-1" />}Crear clínica y generar admin</Button>
        </div>
      </form>
...
function CredsDialog({ creds, onClose }: { creds: any; onClose: () => void }) {
  ...
  const tudo = `Clínica: ${creds.clinica}\nAdmin: ${creds.nome_admin}\nEmail: ${creds.email}\nSenha: ${creds.senha}\nEnlace público: ${creds.link}`;
  const wa = (creds.telefone ?? "").replace(/\D/g, "");
  const waMsg = encodeURIComponent(`Hola ${creds.nome_admin}, su clínica ${creds.clinica} ha sido registrada en OdontoControl AI.\n\nAcceso:\nEmail: ${creds.email}\nContraseña: ${creds.senha}\n\nEnlace público de cita:\n${creds.link}\n\nPor seguridad, cambie la contraseña en el primer acceso.`);
  const waUrl = `https://wa.me/${wa ? (wa.startsWith("55") ? wa : "55" + wa) : ""}?text=${waMsg}`;

  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Credenciales generadas</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="bg-red-50 border border-red-300 rounded p-3 flex gap-2 text-sm text-red-800">
            <AlertTriangle className="size-4 mt-0.5 shrink-0" />
            <div><b>Esta senha só aparece uma vez.</b> Guárdela ahora; no podremos recuperarla.</div>
          </div>
          ...
          <Row label="Enlace público" value={creds.link} onCopy={() => copy(creds.link, "Link")} extra={
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          <Button variant="outline" onClick={() => copy(tudo, "Tudo")}><Copy className="size-4 mr-1" />Copiar todo</Button>
