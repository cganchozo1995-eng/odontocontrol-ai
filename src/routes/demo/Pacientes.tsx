...
const STATUS_LABEL: Record<string, string> = {
  ativo: "Activo", inativo: "Inactivo", novo: "Nuevo", retorno_pendente: "Retorno pendiente",
};
const TAG_LABELS: Record<string, string> = {
  vip: "vip",
  recorrente: "recurrente",
  inativo: "inactivo",
  tratamento_pausado: "tratamiento pausado",
  revisao_pendente: "revisión pendiente",
  novo: "nuevo",
};
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40">
                <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="px-4 py-3 font-semibold">Última cita</th>
                  <th className="px-4 py-3 font-semibold text-right">Citas</th>
                  <th className="px-4 py-3 font-semibold text-right">Historial $</th>
                  ...
                  <tr key={p.id} className="border-t hover:bg-muted/30 cursor-pointer" onClick={() => setSel(p)}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {p.tags.map((t) => (
                          <Badge key={t} variant="outline" className={`text-[10px] ${TAG_PILL[t] ?? ""}`}>{t === "revisao_pendente" ? "revisión pendiente" : t === "tratamento_pausado" ? "tratamiento pausado" : t}</Badge>
                        ))}
                      </div>
                    </td>
                    <td className="px-4 py-3">
    ...
    <>
      ...
      <Tabs defaultValue="geral">
        <TabsList className="grid grid-cols-6 w-full">
          <TabsTrigger value="geral">General</TabsTrigger>
          <TabsTrigger value="hist">Historial</TabsTrigger>
          <TabsTrigger value="trat">Tratamientos</TabsTrigger>
          <TabsTrigger value="orc">Presupuestos</TabsTrigger>
          <TabsTrigger value="fin">Financiero</TabsTrigger>
        ...
        <TabsContent value="trat" className="space-y-2 mt-4">
          {tratPac.length === 0 ? <Empty /> : tratPac.map((t: any) => (
        ...
        <TabsContent value="orc" className="space-y-2 mt-4">
          {orcPac.length === 0 ? <Empty /> : orcPac.map((o: any) => (
...
function Empty() { return <div className="text-center py-8 text-xs text-muted-foreground">Ningún registro.</div>; }
