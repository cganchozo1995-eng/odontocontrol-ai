...
const AI_TITLES: Record<string, string> = {
  ...
  ai2: "8 revisiones pendientes",
  ai3: "5 tratamientos pausados hace +30 días",
  ai4: "6 presupuestos sin respuesta +14 días",
  ai5: "Horario bajo: Martes y jueves 13h–15h",
};
const AI_DESCS: Record<string, string> = {
  ai1: "Recupere facturación reactivando ahora.",
  ai2: "Pacientes que aún no programaron su revisión semestral.",
  ai3: "Continúe los tratamientos y cierre la facturación estancada.",
  ai4: "El seguimiento genera un 32% de aprobación extra.",
  ai5: "Solo el 30% de los cupos ocupados en este intervalo.",
};
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description="La IA escanea sus datos todos los días y muestra exactamente dónde está la facturación estancada."
      ...
      <Card className="border-0 gradient-primary text-white shadow-premium">
        <CardContent className="p-6 flex flex-wrap items-center gap-5">
          ...
          <Button size="lg" variant="secondary" className="bg-white text-primary hover:bg-white/90"
            onClick={() => toast.success("Enviado a todos los contactos (demo).")}>
            Ejecutar todas
          </Button>
        </CardContent>
      </Card>
              ...
              <CardContent className="p-0">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold">{AI_TITLES[o.id] || o.titulo}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">{AI_DESCS[o.id] || o.desc}</div>
                  </div>
                ...
                {isOpen && (
                  <div className="border-t bg-muted/20 p-5 space-y-4 animate-fade-in">
                    ...
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Mensaje sugerido</div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        ...
                        <Button size="sm" variant="outline" onClick={() => toast.info("La IA generó una nueva variación (demo).")}>
                          <Wand2 className="size-3.5 mr-1.5" /> Generar con IA
                        </Button>
                        <Button size="sm" className="gradient-primary text-white" onClick={() => toast.success(`Campaña enviada a ${o.pacientes.length || "todos"} contactos (demo).`)}>
                          <MessageCircle className="size-3.5 mr-1.5" /> Enviar campaña
                        </Button>
                      </div>
                    </div>
                  </div>
      ...
      <Card className="border-0 shadow-card bg-emerald-50/40 border border-emerald-100">
        <CardContent className="p-5 flex items-center gap-3">
          ...
          <div className="text-sm">
            <span className="font-semibold">Datos anonimizados.</span>{" "}
            <span className="text-muted-foreground">La IA se ejecuta solo sobre los datos de su clínica y nunca comparte información con terceros.</span>
          </div>
        </CardContent>
      </Card>
    </div>
