...
const STATUS_HOJE: Record<string, { txt: string; cls: string }> = {
  concluida:       { txt: "Realizado",      cls: "bg-emerald-100 text-emerald-700 border-emerald-200" },
  em_atendimento:  { txt: "En atención", cls: "bg-sky-100 text-sky-700 border-sky-200" },
  agendada:        { txt: "Programado",       cls: "bg-slate-100 text-slate-700 border-slate-200" },
  confirmada:      { txt: "Confirmado",     cls: "bg-violet-100 text-violet-700 border-violet-200" },
  faltou:          { txt: "No asistió",         cls: "bg-red-100 text-red-700 border-red-200" },
};
...
function Page() {
  ...
  const chart14 = Array.from({ length: 14 }).map((_, i) => {
    ...
    return { dia: format(date, "dd/MM", { locale: es }), qtd: 6 + Math.round(Math.sin(i * 0.7) * 4 + (i % 3) * 2 + 5) };
  });

  const top5 = [
    { name: "Limpieza",      value: 38 },
    { name: "Restauración",  value: 26 },
    { name: "Ortodoncia",   value: 22 },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">Buenos días, Dra. Patricia 👋</h1>
          <p className="text-sm text-muted-foreground mt-1">Resumen de la clínica · {format(new Date(), "EEEE, dd 'de' MMMM", { locale: es })}</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2 border-0 shadow-card">
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-bold">Citas — últimos 14 días</h3>
                <p className="text-xs text-muted-foreground mt-0.5">Tendencia diaria de citas realizadas</p>
              </div>
        ...
        <Card className="border-0 shadow-card">
          <CardContent className="p-5">
            <h3 className="font-bold mb-1">Top 5 procedimientos</h3>
            <p className="text-xs text-muted-foreground mb-3">Más realizados este mes</p>
            <div className="h-64">
              <ResponsiveContainer>
                <PieChart>
