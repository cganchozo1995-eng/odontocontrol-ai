...
const ST: Record<string, string> = {
  ...
  realizada:  "bg-emerald-50 border-emerald-400",
  cancelada:  "bg-red-50 border-red-300 opacity-60",
  ausente:     "bg-amber-50 border-amber-400",
};
...
function Page() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader
        title="Agenda"
        description={format(date, "EEEE, dd 'de' MMMM 'de' yyyy", { locale: es })}
        actions={<Button className="gradient-primary text-white shadow-premium"><Plus className="size-4 mr-1.5" />Nueva cita</Button>}
      />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 bg-white rounded-lg border p-1 shadow-card">
          ...
          <Button variant="ghost" size="sm" className="h-8" onClick={() => setDate(new Date())}>Hoy</Button>
                        ...
                        {consH.map((c) => (
                          <div key={c.id} className={`text-xs p-2 rounded-md border-l-4 ${ST[c.status] ?? ST.agendada}`} style={{ borderLeftColor: PROF_COLOR[c.profissional_id] }}>
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant="outline" className="text-[10px] ml-auto">{STATUS_LABELS[c.status] || c.status}</Badge>
                            </div>
              ...
              <div className="grid grid-cols-[80px_repeat(6,minmax(140px,1fr))] min-w-[900px]">
                ...
                {weekDays.map((d) => (
                  <div key={d.toISOString()} className={`border-b border-r p-2 text-center ${isSameDay(d, new Date()) ? "bg-primary/10" : "bg-muted/20"}`}>
                    <div className="text-xs uppercase text-muted-foreground">{format(d, "EEE", { locale: es })}</div>
