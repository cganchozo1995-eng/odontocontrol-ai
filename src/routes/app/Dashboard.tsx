...
function Page() {
  ...
  return (
    <>
      <PageHeader title="Tablero" description="Visión general de su clínica" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        <KpiCard label="Citas hoy" value={consHoje.length} icon={<Calendar className="size-4" />} />
        <KpiCard label="Citas semana" value={consSemana.length} icon={<Calendar className="size-4" />} />
        <KpiCard label="Citas mes" value={consMes.length} icon={<Activity className="size-4" />} />
        <KpiCard label="Facturación mes" value={brl(receitaMes)} icon={<DollarSign className="size-4" />} />
        <KpiCard label="Ticket promedio" value={brl(ticketMedio)} icon={<TrendingUp className="size-4" />} />
        <KpiCard label="Pacientes activos" value={pacAtivos} icon={<Users className="size-4" />} />
        <KpiCard label="Tasa de retorno 90d" value={`${taxaRetorno.toFixed(1)}%`} icon={<Repeat className="size-4" />} />
        <KpiCard label="No-show" value={`${noShowRate.toFixed(1)}%`} icon={<AlertTriangle className="size-4" />} />
        <KpiCard label="Presupuestos pendientes" value={orcPendentes} icon={<Receipt className="size-4" />} />
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-6">
        <Card className="lg:col-span-2">
          <CardContent className="p-5">
            <h3 className="font-semibold mb-4">Atenciones (últimos 7 días)</h3>
            <div className="h-64">
              <ResponsiveContainer>
                <BarChart data={chartSemana}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="consultas" fill="#06B6D4" name="Reservadas" />
                  <Bar dataKey="concluidas" fill="#10B981" name="Realizadas" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="p-5">
            <h3 className="font-semibold mb-4">Principales procedimientos</h3>
            <div className="h-64">
              {topProcs.length ? (
                <ResponsiveContainer>
                  <PieChart>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-4">
        <Card>
          <CardContent className="p-5">
            <h3 className="font-semibold mb-4">Principales profesionales (mes)</h3>
            <div className="h-56">
              {topProfs.length ? (
                <ResponsiveContainer>
        ...
        <Card>
          <CardContent className="p-5">
            <h3 className="font-semibold mb-4 flex items-center gap-2"><Calendar className="size-4 text-primary" />Próximas citas</h3>
        ...
        <Card className={prontuariosPendentes.length ? "border-amber-300" : ""}>
          <CardContent className="p-5">
            <h3 className="font-semibold mb-4 flex items-center gap-2"><FileWarning className="size-4 text-amber-600" />Expedientes pendientes</h3>
            {prontuariosPendentes.length === 0 ? (
              <div className="text-sm text-muted-foreground">¡Todo al día!</div>
            ) : (
              <div className="space-y-2">
      ...
      {isLoading && <div className="text-xs text-muted-foreground mt-4">Actualizando datos…</div>}
    </>
  );
}

function Empty() {
  return <div className="text-sm text-muted-foreground py-8 text-center">Sin datos aún.</div>;
}
