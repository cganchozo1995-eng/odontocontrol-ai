...
function Page() {
  ...
  const remove = async (id: string) => {
    ...
    if (error) toast.error(error.message); else { toast.success("Eliminado"); qc.invalidateQueries({ queryKey: ["tratamento", clinicaId] }); }
  };
  ...
  const advance = async (r: any) => {
    ...
    else { toast.success(`Estado: ${next}`); qc.invalidateQueries({ queryKey: ["tratamento", clinicaId] }); }
  };
  ...
  return (
    <>
      ...
      <div className="grid md:grid-cols-2 gap-4">
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex justify-between items-start gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">Diente: {r.dente ?? "—"} · {r.profissional_nome ?? "—"}</div>
                  </div>
                  <Badge>{r.status}</Badge>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1"><span>Progreso</span><span className="font-medium">{pct}%</span></div>
        ...
        {filtered.length === 0 && <div className="col-span-full text-center text-muted-foreground py-10">Ningún tratamiento.</div>}
      </div>

      <FormDialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEdit(null); }} title={edit?.id ? "Editar tratamiento" : "Nuevo tratamiento"} wide>
        <SimpleForm
          initial={edit ?? {}}
          busy={busy}
          onSubmit={save}
          fields={[
            ...
            { name: "profissional_id", label: "Profesional", type: "select", options: profs.map((p: any) => ({ value: p.id, label: p.nome })) },
            { name: "status", label: "Estado", type: "select", options: STATUS },
            { name: "data_inicio", label: "Inicio", type: "date" },
            { name: "data_conclusao", label: "Conclusión", type: "date" },
            { name: "dente", label: "Diente/Local" },
            { name: "valor_total", label: "Valor total", type: "number", step: "0.01" },
            { name: "descricao", label: "Etapas (separar por ; o nueva línea)", type: "textarea", col: 2, required: true },
