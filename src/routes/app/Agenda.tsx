...
function Page() {
  ...
  const onDrag = async (r: DropResult) => {
    ...
    else { toast.success("Reprogramado para " + format(new Date(novaData + "T12:00"), "dd/MM")); qc.invalidateQueries({ queryKey: ["consultas", clinicaId] }); }
  };
  ...
  return (
    <>
      <PageHeader title="Agenda" description="Calendario inteligente · arrastre para reprogramar" actions={
      ...
      <FormDialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEdit(null); }} title={edit?.id ? "Editar cita" : "Nueva cita"} wide>
        <SimpleForm
          ...
          fields={[
            ...
            { name: "profissional_id", label: "Profesional", type: "select", required: true, col: 2, options: profs.map((p: any) => ({ value: p.id, label: p.nome })) },
            ...
            { name: "prontuario", label: "Expediente", type: "textarea", col: 2 },
          ]}
        />
      </FormDialog>
    </>
