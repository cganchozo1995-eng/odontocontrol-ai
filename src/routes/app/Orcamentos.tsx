...
function Page() {
  ...
  const remove = async (id: string) => {
    ...
    if (error) toast.error(error.message); else { toast.success("Eliminado"); qc.invalidateQueries({ queryKey: ["orcamento", clinicaId] }); }
  };

  return (
    <>
      <PageHeader title="Presupuestos" description="Planes financieros con aprobación y PDF" actions={
        <Button onClick={openNew}><Plus className="size-4 mr-1" />Nuevo</Button>
      } />
      <DataTable
        rows={rows as any[]}
