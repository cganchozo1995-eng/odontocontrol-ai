...
const ROLES = [
  ...
  { value: "auxiliar", label: "Auxiliar" }, { value: "financeiro", label: "Financiero" },
];
...
function Page() {
  ...
  const save = async (v: any) => {
    ...
    try {
      if (edit?.id) {
        ...
        if (error) throw error;
        toast.info("Miembro agregado. Pídale que cree una cuenta con este correo electrónico.");
      }
  ...
  return (
    <>
      <PageHeader title="Equipo" description="Miembros y permisos" actions={
        <Button onClick={() => { setEdit({}); setOpen(true); }}><Plus className="size-4 mr-1" />Nuevo miembro</Button>
      } />
      <DataTable
        rows={rows as any[]}
        ...
        columns={[
          ...
          { key: "ativo", header: "Estado", render: (r: any) => r.ativo ? <Badge variant="default">Activo</Badge> : <Badge variant="secondary">Inactivo</Badge> },
      ...
      />
      <FormDialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEdit(null); }} title={edit?.id ? "Editar miembro" : "Nuevo miembro"}>
        <SimpleForm
