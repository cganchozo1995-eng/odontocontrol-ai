  ...
  return (
    <div className="orc-pdf p-8 max-w-3xl mx-auto bg-white text-black">
      ...
      <table className="w-full text-sm mb-4 border-collapse">
        ...
        <tbody>
          {itens.length === 0 && <tr><td colSpan={4} className="py-4 text-center text-gray-400">Sin ítems</td></tr>}
          {itens.map((it, i) => (
            <tr key={i} className="border-b">
              ...
              <td className="text-right">{money((it.valor ?? 0) * (it.qtd ?? 1))}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <footer className="mt-12 pt-12 grid grid-cols-2 gap-8 text-xs">
        <div className="border-t pt-2 text-center">Firma del paciente</div>
        <div className="border-t pt-2 text-center">Firma del responsable</div>
      </footer>
    </div>
  );
}
