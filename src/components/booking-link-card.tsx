import { useRef } from "react";
import { QRCodeCanvas } from "qrcode.react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Copy, ExternalLink, Download, LinkIcon } from "lucide-react";
import { toast } from "sonner";

export function BookingLinkCard({ slug }: { slug: string | null | undefined }) {
  const ref = useRef<HTMLDivElement>(null);

  if (!slug) {
    return (
      <Card><CardContent className="p-5 text-sm text-muted-foreground">
        Defina el slug público de la clínica en Configuración para habilitar la cita en línea.
      </CardContent></Card>
    );
  }

  const publicBase = (import.meta.env.VITE_PUBLIC_APP_URL as string | undefined)?.replace(/\/$/, "");
  const previewOrigin = typeof window !== "undefined" ? window.location.origin : "";
  // Prefer the configured public domain (published/custom domain). Fall back to current origin only if it's not a preview URL.
  const origin = publicBase || (previewOrigin && !previewOrigin.includes("id-preview--") ? previewOrigin : previewOrigin);
  const url = `${origin}/agendar/${slug}`;

  const copy = async () => {
    await navigator.clipboard.writeText(url);
    toast.success("Enlace copiado");
  };
  const open = () => window.open(url, "_blank", "noopener");
  const download = () => {
    const canvas = ref.current?.querySelector("canvas") as HTMLCanvasElement | null;
    if (!canvas) return;
    const a = document.createElement("a");
    a.href = canvas.toDataURL("image/png");
    a.download = `qr-agendamento-${slug}.png`;
    a.click();
  };

  return (
    <Card className="border-primary/30 bg-gradient-to-br from-primary/5 to-transparent">
      <CardContent className="p-5">
        <div className="flex flex-col md:flex-row gap-5 items-start">
          <div ref={ref} className="bg-white p-3 rounded-lg border shadow-sm">
            <QRCodeCanvas value={url} size={128} level="M" />
          </div>
          <div className="flex-1 space-y-3 min-w-0">
            <div className="flex items-center gap-2 text-primary font-semibold">
              <LinkIcon className="size-4" />
              Enlace público de cita
            </div>
            <div className="font-mono text-xs bg-muted rounded p-2 break-all">{url}</div>
            <div className="flex flex-wrap gap-2">
              <Button size="sm" onClick={copy}><Copy className="size-4 mr-1" />Copiar enlace</Button>
              <Button size="sm" variant="outline" onClick={open}><ExternalLink className="size-4 mr-1" />Abrir</Button>
              <Button size="sm" variant="outline" onClick={download}><Download className="size-4 mr-1" />Descargar QR</Button>
            </div>
          </div>
        </div>
      </CardContent>
    </Card>
  );
}
