...
export function CrudPage<T extends Record<string, any>>({
  ...
  const remove = async (id: string) => {
    ...
    else { toast.success("Eliminado"); qc.invalidateQueries({ queryKey: [table, clinicaId] }); }
  };
  ...
  return (
    <>
      <PageHeader title={title} description={description} actions={
        <Button onClick={() => { setEdit(defaults ?? {}); setOpen(true); }}><Plus className="size-4 mr-1" />Nuevo</Button>
      } />
      <DataTable rows={rows as any} columns={cols as any} searchKeys={searchKeys as any} />
      <FormDialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEdit(null); }} title={edit?.id ? "Editar" : "Nuevo"} wide>
